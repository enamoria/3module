...
if __name__ == "__main__":
    ...
    dev_sents = read_data("/home/enamoria/Desktop/workspace/evaluation/ws/data/dev.txt")
    ...
    X_train = [sent2features(sent, mode="train") for sent in train_sents]  # if len(sent) >= 1 and len(sent[0]) >= 1 and sent[0][0]]
    y_train = [sent2labels(sent) for sent in train_sents]  # len(sent) >= 1 and len(sent[0]) >= 1 and sent[0][0]]

    X_dev = [sent2features(sent, mode="dev") for sent in dev_sents]  # if len(sent) >= 1 and len(sent[0]) >= 1 and sent[0][0]]

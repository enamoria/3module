...
"""
    Read data into acceptable format for pycrfsuite: [[sentence = list of tuple (token a.k.a word, POS-tag, name entity)]]
"""
...
class Reader:
    ...
    def reader_10000(self):
        ...
        print("'vi' at", self.datapath)

        filelist = sorted([filename for filename in os.listdir(self.datapath) if os.path.isfile(self.datapath + "/" + filename)])

        data = []  # data will be stored here

        # THIS COMMENT WAS ADDED IN 17 July: Địt mẹ mày code như lồn vậy
        for file in filelist:
            with open(self.datapath + "/" + file, "rb") as f:
                ...
                #     if line == '':
                #         print("FUCK")
                raw_text = [(line.decode("utf8").strip("\n").strip("\r").strip(" ").replace("   ", " ").replace("  ", " ")).split(" ") for line in f.readlines() if line != '\n' or not line]
                # print(raw_text)
        ...
        print("Done reading 'vi' data,", len(filelist), "file found, with", len(data), "sentences")
    ...
    def reader_VLSP(self):
        ...
        filelist = sorted([os.path.isfile(self.datapath + "/" + filename) for filename in os.listdir(self.datapath)])
    ...
    def normalize(self):  # , data):
        print("=======================================")
        print("Running normalize ... Old data will be overwritten")
